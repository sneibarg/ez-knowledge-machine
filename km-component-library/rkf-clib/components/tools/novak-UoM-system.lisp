...
; kmunits.lsp               Gordon S. Novak Jr.             ; 08 Dec 05

; Copyright (c) 2005 Gordon S. Novak Jr. and The University of Texas at Austin.
; All rights reserved.

; 25 Aug 05; 28 Oct 05; 21 Nov 05; 06 Dec 05

#|
I have written some functions in /u/novak/glisp/units.lsp
to handle translation between algebraic unit expressions and KM.

(defkmunit '|*square-meter| '(* meter meter))

This defines a KM unit in terms of an algebraic unit.
A file of such definitions can be used in addition to units.lsp
to define the KM units, without modifying units.lsp .

(kmifyunit '(* meter meter))  =>  |*square-meter|

kmifyunit will translate an algebraic unit (if possible) into
a KM unit.

(glsimplifyunit '|*square-meter|)  =>  SQUARE-METER

glsimplifyunit can be used to translate KM units to algebraic units.
The KM units can also be used directly with the unit functions.
The KM units are defined as two units systems, KM-SI and KM-ENGLISH.
|#

(defvar *kmunitequivalents* nil)

(setq *kmunitequivalents* '(
(|*day| day)                        ; UoM-Duration UoM-Time
(|*week| week)
(|*month| month)
(|*year| year)
(|*hour| hour)
...
(|*nanosecond| nanosecond)
                                    ; UoM-Acceleration
(|*centimeter-per-second-squared| centimeters-per-second-squared)
(|*foot-per-second-squared| feet-per-second-squared)
(|*meter-per-second-squared| meters-per-second-squared)
(|*kilometer-per-second-squared| (/ kilometer (* second second)))
(|*mile-per-second-squared| (/ mile (* second second)))

(|*degree| degree)                        ; UoM-Angle
(|*radian| radian)
(|*grad| grad)

(|*square-inch| square-inch)                        ; UoM-Area
(|*square-foot| square-foot)
(|*square-meter| square-meter)

(|*candela| candela)                        ; UoM-Brightness

(|*degree| degree)                        ; UoM-Coordinate
(|*arcminute| arcminute)
(|*arcsecond| arcsecond)

(|*gram/liter| (/ gram liter))                        ; UoM-Density
(|*kilogram/gallon| (/ kilogram gallon))   ; ???

(|*newton| newton)                        ; UoM-Force

(|*per-hour| (/ 1 hour))                        ; UoM-Frequency
(|*per-minute| (/ 1 minute))
(|*per-second| hertz)
(|*hertz| hertz)
(|*kilohertz| kilohertz)
...
; (|*kilocalorie/mole| (/ (* kilo calorie) mole))    ; UoM-Intensity

(|*kilometer| kilometer)                        ; UoM-Length
(|*inch| inch)
(|*foot| foot)
(|*mile| mile)
(|*yard| yard)
...
(|*lumen| (* candela steradian))                        ; UoM-Luminous-Flux
; (|*candelas-per-steradian| )  ; ???

(|*candela| candela)                        ; UoM-Luminous-Intensity

(|*gram| gram)                        ; UoM-Mass UoM-Weight ???
(|*kilogram| kilogram)
(|*dalton| dalton)
(|*ounce| ounce)
(|*pound| pound)
...
(|*pounds-per-square-inch| pounds-per-square-inch )    ; UoM-Pressure
(|*pascal| pascal)
                   ; UoM-Quantity
(|*mole| mole)     ; ??? I have mole as a category 'substance'
(|*pair| pair)
(|*dozen| dozen)
(|*score| score)
(|*gross| gross)

(|*mile-per-hour| miles-per-hour)        ; UoM-Rate UoM-Velocity
(|*foot-per-second| feet-per-second)
(|*kilometer-per-hour| kilometers-per-hour)
(|*meter-per-second| meters-per-second)
(|*centimeter-per-second| centimeters-per-second)  ; add s
(|*mile-per-second| miles-per-second)  ; add s

(|*revolutions-per-minute| (/ 1 minute))       ; UoM-Rotational-Rate

; (|*fahrenheit| fahrenheit)                        ; UoM-Temperature
; (|*celsius| celsius)
(|*kelvin| degree-kelvin)

(|*no-unit| unity)                        ; UoM-Unitless
(|*unity| unity)
(|*volt| volt)                        ; UoM-Voltage

(|*milliliter| milliliter)                        ; UoM-Volume
(|*liter| liter)
(|*gallon| gallon)
(|*cubic-meter| (* meter meter meter))
...
(|*electronic-charge| (* 1.6021892e-19 coulomb))      ; UoM-Charge

; (|*molar| )                        ; UoM-Concentration

(|*ampere| ampere)                        ; UoM-Electric-Current
(|*milliampere| milliampere)

(|*ohm| ohm)                        ; UoM-Electric-Resistance

(|*calorie| calorie)                        ; UoM-Energy
(|*joule| joule)
(|*btu| btu)
(|*mole| mole)
(|*molar| molar)
...
(dolist (x *kmunitequivalents*) (defkmunit (first x) (second x)))

(defvar *kmunitclassequivalents* nil)
(setq *kmunitclassequivalents* '(
(|UoM-Energy| energy)
(|UoM-Electric-Resistance| resistance)
(|UoM-Electric-Current| current)
(|UoM-Charge| charge)
(|UoM-Worth| money)
(|UoM-Volume| volume)
(|UoM-Voltage| electric-potential)
(|UoM-Velocity| speed)           ; UoM-Rate
(|UoM-Unitless| dimensionless)   ; UoM-Angle UoM-Quantity UoM-Coordinate
(|UoM-Time| time)
(|UoM-Duration| time)
(|UoM-Temperature| temperature)
(|UoM-Pressure| pressure)
(|UoM-Power| power)
(|UoM-Mass| mass)
(|UoM-Luminous-Intensity| luminosity)
(|UoM-Length| length)
(|UoM-Frequency| frequency)      ; UoM-Rotational-Rate
(|UoM-Force| force)              ; UoM-Weight
(|UoM-Density| density)
(|UoM-Area| area)
(|UoM-Acceleration| acceleration)
(|UoM-Concentration| concentration)
; (|UoM-Pixel-Length| )
; (|UoM-Optical-Resolution| )
; (|UoM-Data-Transfer-Rate| )
; (|UoM-Cost-Rate| )
; (|UoM-Computer-Storage-Size| )
; (|UoM-Bit-Length| )
; (|UoM-Weight| )
; (|UoM-Rate| speed)
; (|UoM-Concentration| )
